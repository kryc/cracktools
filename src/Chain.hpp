...
public:
    Chain(void) = default;
    Chain(__uint128_t Index, std::string_view Start, std::string_view End, size_t Length):
        m_Index(Index), m_Start(Start), m_End(End), m_Length(Length){};
    Chain(__uint128_t Index, std::string_view Start, size_t Length):
        m_Index(Index), m_Start(Start), m_Length(Length){};
    void SetIndex(const __uint128_t Index) { m_Index = Index; };
    ...
    const __uint128_t Index(void) const { return m_Index; };
